...
import 'package:super_context_menu/super_clipboard/src/reader_internal.dart';
import 'package:super_native_extensions/raw_drag_drop.dart' as raw;
import 'package:super_native_extensions/raw_clipboard.dart' as raw;

import '../../super_clipboard/src/format.dart';
import '../../super_clipboard/src/reader.dart';
import 'util.dart';
import 'drop.dart';
...
class _DropItem extends DropItem {
  ...
  @override
  List<PlatformFormat> get platformFormats => _reader?.platformFormats ?? _item.formats;
...
class _DropSession extends DropSession {
  ...
  Future<void> updateItems(
    ...
    for (final (index, item) in items.indexed) {
      ...
      existing ??= current.firstWhereOrNull((element) => element._item.itemId == item.itemId);
      if (existing != null) {
        existing._item = item;
        _items.add(existing);
      } else {
    ...
    final itemsNeedingReaders =
        _items.where((element) => element._item.readerItem != null && element._reader == null).toList(growable: false);
  ...
  Future<void> performDrop({
    ...
    if (_currentDropRegion?.attached == true) {
      await _currentDropRegion?.onPerformDrop(PerformDropEvent(
          session: this, position: DropPosition.forRenderObject(location, _currentDropRegion!), acceptedOperation: acceptedOperation));
    }
  }
  ...
  Future<raw.ItemPreview?> getDropItemPreview(raw.ItemPreviewRequest request) async {
    final item = _items.firstWhereOrNull((element) => element._item.itemId == request.itemId);
    if (item != null && _currentDropRegion != null) {
      final req = _DropItemPreviewRequest(item: item, request: request);
      final response = await _currentDropRegion?.onGetDropItemPreview?.call(this, req);
      if (response != null) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty('allowedOperations', _allowedOperations));
...
class _DropContextDelegate extends raw.DropContextDelegate {
  ...
  Future<raw.DropOperation> onDropUpdate(raw.DropEvent event) async {
    final session = _sessions.putIfAbsent(event.sessionId, () => _DropSession());
  ...
  @override
  Future<raw.ItemPreview?> onGetItemPreview(raw.ItemPreviewRequest request) async {
...
class DropFormatRegistry {
  ...
  DropFormatRegistration registerPlatformDropFormats(List<PlatformFormat> formats) {
  ...
  void _updateIfNeeded() async {
    ...
    if (_lastRegisteredFormats == null || !eq(formats, _lastRegisteredFormats!)) {
...
class RenderDropRegionBox extends RenderProxyBoxWithHitTestBehavior with RenderDropRegion {
  RenderDropRegionBox({
...
class RenderDropMonitorBox extends RenderProxyBoxWithHitTestBehavior with RenderDropMonitor {
